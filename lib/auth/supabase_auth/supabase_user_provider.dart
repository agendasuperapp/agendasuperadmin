...
class AgendaSuperAdminSupabaseUser extends BaseAuthUser {
  AgendaSuperAdminSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> agendaSuperAdminSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AgendaSuperAdminSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
